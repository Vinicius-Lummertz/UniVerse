...
def get_profile_pic_path(instance, filename):
    # Ex: users/vinicius/profile/foto.jpg
    return f'users/{instance.user.username}/profile/{filename}'

def get_cover_photo_path(instance, filename):
    # Ex: users/vinicius/cover/capa.jpg
    return f'users/{instance.user.username}/cover/{filename}'

def get_community_image_path(instance, filename):
    # Ex: communities/devs-python/icon/logo.png
    return f'communities/{instance.name}/icon/{filename}'

def get_community_cover_path(instance, filename):
    # Ex: communities/devs-python/cover/banner.jpg
    return f'communities/{instance.name}/cover/{filename}'

def get_post_image_path(instance, filename):
    # Ex: posts/vinicius/images/ferias.jpg
    return f'posts/{instance.owner.username}/images/{filename}'

def get_post_video_path(instance, filename):
    # Ex: posts/vinicius/videos/tutorial.mp4
    return f'posts/{instance.owner.username}/videos/{filename}'

def get_post_attachment_path(instance, filename):
    # Ex: posts/vinicius/files/documento.pdf
    return f'posts/{instance.owner.username}/files/{filename}'
...
class Posts(models.Model):
    ...
    # --- USANDO AS NOVAS FUNÇÕES DE CAMINHO ---
    image = models.ImageField(upload_to=get_post_image_path, blank=True, null=True) 
    video = models.FileField(upload_to=get_post_video_path, blank=True, null=True)
    attachment = models.FileField(upload_to=get_post_attachment_path, blank=True, null=True)
...
class Profile(models.Model):
    ...
    cover_photo = models.ImageField(upload_to=get_cover_photo_path, null=True, blank=True)
...
class Announcement(models.Model):
    ...
    class Meta:
        ordering = ['-timestamp']
    def __str__(self):
        return f"Anúncio de {self.author.username}"
...
class Community(models.Model):
    ...
    privacy = models.CharField(max_length=10, choices=[('public', 'Pública'), ('private', 'Privada')], default='public')
    related_course = models.CharField(max_length=200, blank=True) 
    
    # --- CAMINHOS ORGANIZADOS ---
    community_image = models.ImageField(upload_to=get_community_image_path, null=True, blank=True)
    cover_image = models.ImageField(upload_to=get_community_cover_path, null=True, blank=True)
...
class CommunityMembership(models.Model):
    ...
    status = models.CharField(max_length=10, choices=[('pending', 'Pendente'), ('approved', 'Aprovado')], default='approved')
